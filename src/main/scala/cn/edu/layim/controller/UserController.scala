...
import cn.edu.layim.service.CookieService
...
class UserController @Autowired() (userService: UserService, cookieService: CookieService) {
  ...
  def findMyGroups(
      @RequestParam(value = "page", defaultValue = "1") page: Int,
      @RequestParam(value = "createId", required = true) createId: Integer
  ): String = {
    ...
    val groupNews =
      groups.toArray.filter(x => x.asInstanceOf[GroupList].getCreateId.equals(createId))
  ...
  def getOffLineMessage(request: HttpServletRequest): String = {
    ...
    receives.asScala.foreach { receive =>
      {
  ...
  def register(@RequestBody user: User, request: HttpServletRequest): String = {
    if (userService.saveUser(user, request))
      gson.toJson(new ResultSet(SystemConstant.SUCCESS, SystemConstant.REGISTER_SUCCESS))
  ...
  def createGroup(@RequestBody groupList: GroupList): String = {
    ...
    if (userService.addGroupMember(ret, groupList.getCreateId)) {
      return gson.toJson(
        new ResultSet(SystemConstant.SUCCESS, SystemConstant.CREATE_GROUP_SUCCCESS)
      )
    }
